...
async function _iniciarWizard(cuartelId) {
  _personal  = await obtenerPersonalConFallback(cuartelId)
  _vehiculos = await obtenerVehiculosConFallback(cuartelId)
...
function paso1Html() {
  ...
  const opsPersonal   = _personal.map(p =>
    `<option value="${p.id}" data-nombre="${p.grado} ${p.nombre_completo}">
      ${p.grado} ${p.nombre_completo}
    </option>`).join('')
  const opsVehiculo   = _vehiculos.map(v =>
    `<option value="${v.id}" data-patente="${v.patente}">
      ${v.patente} — ${v.tipo || ''}
    </option>`).join('')
