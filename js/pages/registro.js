...
// FIX: usuario sin cuartel asignado puede seleccionar cuartel
//      antes de registrar el servicio
// ============================================================
...
const DELITOS = {
  ...
  trafico_migrantes: 'Tráfico de migrantes',
...
// ── Helper: resuelve el cuartelId activo ─────────────────────
...
async function renderRegistro() {
  ...
  // Sin cuartel asignado → mostrar selector primero
  if (APP.sinCuartel) {
      ...
      <div class="container-sm">
        <div class="card gap3">
          ...
          <p style="font-size:.8rem;color:var(--muted)">
            Su usuario no tiene un cuartel asignado. Seleccione el cuartel
            correspondiente al servicio que va a registrar.
          </p>
          <div class="campo">
            <label>Cuartel</label>
  ...
  // Con cuartel asignado → flujo normal
...
async function seleccionarCuartelManual(cuartelId) {
  ...
  if (wrap) wrap.innerHTML = `<div class="cargando" style="padding:1.5rem">Cargando datos del cuartel...</div>`
  await _iniciarWizard(cuartelId)
}

async function _iniciarWizard(cuartelId) {
  _personal  = []   // ya no se usan en campos libres
  _vehiculos = []   // ya no se usan en campos libres
...
// ── Render del wizard ─────────────────────────────────────────
function renderPaso() {
  // Si hay selector de cuartel activo, renderizar dentro del wrap
...
function paso1Html() {
  ...
  return `
    ...
    <p style="font-family:var(--mono);font-size:.68rem;color:var(--muted);margin-bottom:1.5rem">
      ...
      ${!navigator.onLine
        ? '<span style="color:var(--amarillo);margin-left:.5rem">● MODO OFFLINE</span>'
        : ''}
    </p>
    ...
    <div id="m5-detalle" style="${_m5Activo ? '' : 'display:none'};margin-bottom:1rem">
      <div class="campo">
        ...
        <input id="m5-unidad" type="text"
          placeholder="Ej: Regimiento Huamachuco N°23..."
          value="${_servicio.unidad_ffaa}"
          onchange="_servicio.unidad_ffaa=this.value" />
      </div>
    </div>
    ...
    <div class="g2 gap">
      <div class="campo">
        ...
        <input id="f-nombre-jefe" type="text"
      ...
      <div class="campo">
        ...
        <input id="f-vehiculo-texto" type="text"
          placeholder="Ej: BPZK-45 — Camioneta"
...
function irPaso2() {
  // Leer campos libres directamente
...
function paso2Html() {
  const timelineHtml = _tareas.length
    ? `<div class="timeline" id="timeline">
        ${_tareas.map((t, i) => tareaHtml(t, i)).join('')}
       </div>`
    : `<div style="color:var(--muted);font-family:var(--mono);font-size:.72rem;
        letter-spacing:1px;padding:1rem 0;text-align:center">
        SIN TAREAS AGREGADAS AÚN
       </div>`
  const botonesHtml = Object.entries(TIPOS_TAREA).map(([cat, tipos]) => `
    <div class="cat-label-bar">
      ${cat === 'territorial' ? '🗺 TERRITORIAL'
        : cat === 'operativa' ? '⚡ OPERATIVA'
        : '📋 ADMINISTRATIVA'}
    </div>
    <div class="grid-tareas">
      ${tipos.map(t => `
        <button class="btn-tarea"
          onclick="agregarTarea('${t.tipo}','${cat}')">
  ...
  return `
    ...
    <p style="font-family:var(--mono);font-size:.68rem;color:var(--muted);
      margin-bottom:1.25rem">Agregue las tareas en el orden que ocurrieron</p>
...
function agregarTarea(tipo, categoria) {
  ...
  _tareas.push({
    ...
    resultado: {
      tipo_resultado: '', tipo_delito: '', numero_parte: '',
      valor_total_uf: 0, como_se_detecto: '', como_se_actuo: '',
      info_para_csf: '', detenidos: [], especies: [],
    },
...
function eliminarTarea(id) {
  ...
  if (wrap) wrap.innerHTML = _tareas.length
    ...
    : `<div style="color:var(--muted);font-family:var(--mono);font-size:.72rem;
        padding:1rem 0;text-align:center">SIN TAREAS AGREGADAS AÚN</div>`
}
...
function tareaHtml(t, index) {
  ...
  const necesitaRes   = [
    'control_vehicular_con','detencion','incautacion',
    'hallazgo_encargo','bien_cot_sin_detenido',
  ].includes(t.tipo)
    ...
    <div class="tarea-card cat-${t.categoria}" id="tarea-${t.id}">
      ...
      <div class="campo sm gap">
        ...
        <select onchange="syncTarea('${t.id}','punto_id',this.value)">
          ...
          ${puntosFiltrados.map(p =>
            `<option value="${p.id}">${p.nombre} — ${p.pais}</option>`
          ).join('')}
        </select>
      </div>` : ''}
      ...
      <div class="g2 gap">
        <div class="campo sm"><label>Institución</label>
          <select onchange="syncTarea('${t.id}','institucion',this.value)">
            ...
            ${['PNP Tacna','PNP Arica','PDI','Aduana','SML',
               'Ejército','Armada','FACh','Otra']
      ...
      <label class="check-label" style="margin-top:.75rem">
        <input type="checkbox" ${t.tiene_hallazgo ? 'checked' : ''}
          onchange="syncTarea('${t.id}','tiene_hallazgo',this.checked);
                    toggleHallazgo('${t.id}',this.checked)" />
        <span style="color:var(--amarillo)">
          ¿Hay hallazgo relevante para inteligencia / CSF?
        </span>
      </label>
      <div id="hallazgo-bloque-${t.id}" class="hallazgo-bloque"
        ...
        <div class="g2 gap">
          ...
          <div class="campo sm gc12"><label>Descripción detallada</label>
            <textarea
              placeholder="Hora exacta, ubicación, características..."
      ...
      <div class="campo sm" style="margin-top:.75rem">
        ...
        <input type="text" placeholder="Estado del punto, novedades..."
    ...
    <div class="resultado-bloque" id="res-bloque-${t.id}">
      ...
      <div class="g2 gap">
        <div class="campo sm"><label>Tipo de Resultado</label>
          <select onchange="syncTarea('${t.id}','res.tipo_resultado',this.value);
                            syncTarea('${t.id}','tiene_resultado',true)">
            <option value="">Seleccionar...</option>
        ...
        <div class="campo sm"><label>Tipo de Delito</label>
          <select onchange="syncTarea('${t.id}','res.tipo_delito',this.value)">
            ...
            ${Object.entries(DELITOS)
              .map(([k,v]) => `<option value="${k}">${v}</option>`).join('')}
          </select></div>
        <div class="campo sm"><label>N° Parte (obligatorio)</label>
      ...
      <div class="campo sm"><label>Información útil para la CSF</label>
        <textarea
          placeholder="Modus operandi, datos de interés, recomendaciones..."
          onchange="syncTarea('${t.id}','res.info_para_csf',this.value)">
        </textarea></div>
    </div>`
}
...
function agregarEspecie(tareaId) {
  ...
  t.resultado.especies.push({
    ...
    tipo_vehiculo: '', marca: '', modelo: '', patente: '',
    tenia_encargo: false,
  })
  renderDetenidosEspecies(tareaId)
}
        ...
        <div class="campo sm"><label>Nacionalidad</label>
          <select onchange="syncDetenido('${tareaId}','${d.id}','nacionalidad',this.value)">
            ...
            ${NACIONALIDADES.map(n =>
              `<option ${d.nacionalidad===n?'selected':''}>${n}</option>`).join('')}
          </select></div>
        ...
        <div class="campo sm" style="grid-column:1/-1">
          <label class="check-label" style="margin:0">
            ...
            <span style="color:var(--rojo);font-weight:600">
              ⚑ Objetivo con alerta internacional (DFO-06)
            </span>
          </label></div>
  ...
  if (eWrap) eWrap.innerHTML = t.resultado.especies.map(e => `
    <div style="background:var(--bg2);padding:.75rem;border-radius:var(--radius);margin-bottom:.4rem">
        ...
        <div class="campo sm"><label>Tipo</label>
          <select onchange="syncEspecie('${tareaId}','${e.id}','tipo',this.value)">
            ${['droga','dinero','vehiculo','arma','mercaderia','otro']
              .map(o => `<option value="${o}" ${e.tipo===o?'selected':''}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`)
              .join('')}
          </select></div>
      ...
      ${e.tipo==='droga'?`<div class="g3">
        <div class="campo sm"><label>Tipo droga</label>
          <input type="text" value="${e.tipo_droga}" placeholder="Cocaína..."
            onchange="syncEspecie('${tareaId}','${e.id}','tipo_droga',this.value)"/></div>
        <div class="campo sm"><label>Cantidad</label>
          <input type="number" step="0.001" value="${e.cantidad_droga}"
            onchange="syncEspecie('${tareaId}','${e.id}','cantidad_droga',this.value)"/></div>
        <div class="campo sm"><label>Unidad</label>
          <select onchange="syncEspecie('${tareaId}','${e.id}','unidad_droga',this.value)">
            <option>kg</option><option>g</option><option>unidades</option>
          </select></div>
      </div>`:''}
      ${e.tipo==='arma'?`<div class="g3">
        <div class="campo sm"><label>Tipo arma</label>
          <input type="text" value="${e.tipo_arma}" placeholder="Pistola..."
            onchange="syncEspecie('${tareaId}','${e.id}','tipo_arma',this.value)"/></div>
        <div class="campo sm"><label>Cantidad</label>
          <input type="number" value="${e.cantidad_armas}"
            onchange="syncEspecie('${tareaId}','${e.id}','cantidad_armas',+this.value)"/></div>
        <div class="campo sm"><label>N° Serie</label>
          <input type="text" value="${e.numero_serie}"
            onchange="syncEspecie('${tareaId}','${e.id}','numero_serie',this.value)"/></div>
      </div>`:''}
      ${e.tipo==='vehiculo'?`<div class="g3">
        <div class="campo sm"><label>Tipo</label>
          <input type="text" value="${e.tipo_vehiculo}" placeholder="Camioneta..."
            onchange="syncEspecie('${tareaId}','${e.id}','tipo_vehiculo',this.value)"/></div>
        <div class="campo sm"><label>Marca</label>
          <input type="text" value="${e.marca}"
            onchange="syncEspecie('${tareaId}','${e.id}','marca',this.value)"/></div>
        <div class="campo sm"><label>Patente</label>
          <input type="text" value="${e.patente}"
            onchange="syncEspecie('${tareaId}','${e.id}','patente',this.value)"/></div>
      </div>`:''}
      ${['mercaderia','otro'].includes(e.tipo)?`
        <div class="campo sm"><label>Descripción</label>
          <input type="text" value="${e.descripcion}"
            onchange="syncEspecie('${tareaId}','${e.id}','descripcion',this.value)"/></div>`:''}
    </div>`).join('')
}

function syncDetenido(tId, dId, campo, val) {
  const t = _tareas.find(t => t.id === tId)
  const d = t?.resultado.detenidos.find(d => d.id === dId)
  if (d) d[campo] = val
}
function eliminarDetenido(tId, dId) {
  const t = _tareas.find(t => t.id === tId)
  if (t) {
    t.resultado.detenidos = t.resultado.detenidos.filter(d => d.id !== dId)
    renderDetenidosEspecies(tId)
  }
}
function syncEspecie(tId, eId, campo, val) {
  const t = _tareas.find(t => t.id === tId)
  const e = t?.resultado.especies.find(e => e.id === eId)
  if (e) e[campo] = val
}
function eliminarEspecie(tId, eId) {
  const t = _tareas.find(t => t.id === tId)
  if (t) {
    t.resultado.especies = t.resultado.especies.filter(e => e.id !== eId)
    renderDetenidosEspecies(tId)
  }
}
...
function detectarTurnoNocturno() {
  const hI    = _servicio.hora_inicio || ''
  ...
  if (hT < hI) {
    ...
    aviso.style.display = ''
    aviso.textContent   =
      `🌙 Turno nocturno detectado — Duración calculada: ${h}h ${m}m (cruza medianoche)`
  } else {
...
function paso4Html() {
  ...
  return `
    ...
    <div class="alerta-cohecho" style="margin-bottom:1rem">
      ...
      <div class="alerta-cohecho-sub">
        Este servicio registra un procedimiento por cohecho.
        Al enviarlo se notificará la cadena de mando.
      </div>
    </div>` : ''}
    <div class="resumen-lista gap2">
      ...
      ${_servicio.tiene_mision_ffaa
        ? filaResumen('M5 FF.AA.',
            `<span style="color:var(--azul);font-weight:600">
              ACTIVO — ${_servicio.unidad_ffaa || 'Decreto N°78'}
            </span>`)
        : ''}
      ${filaResumen('Tareas',
          `<span style="color:var(--verde)">${_tareas.length} registradas</span>`)}
      ${conResult
        ? filaResumen('Con resultado',
            `<span style="color:var(--rojo)">${conResult}</span>`)
        : ''}
      ${conHallazgo
        ? filaResumen('Hallazgos intel.',
            `<span style="color:var(--amarillo)">${conHallazgo}</span>`)
        : ''}
      ${!navigator.onLine
        ? filaResumen('Conexión',
            '<span style="color:var(--amarillo)">⚠ OFFLINE — Se sincronizará automáticamente</span>')
        : ''}
    </div>
    <div class="aviso" style="margin-top:1rem">
      ⚠ Una vez enviado no puede modificarse sin autorización del Administrador.
    </div>
    <div class="flex-gap" style="margin-top:1.5rem">
      <button class="btn btn-secundario" onclick="_paso=3;renderPaso()">← VOLVER</button>
      <button class="btn btn-primario" id="btn-enviar" onclick="iniciarEnvio()">
        ▶ ENVIAR SERVICIO
      </button>
    </div>`
}
...
function filaResumen(key, val) {
  return `<div class="resumen-fila">
    <span class="resumen-key">${key}</span><span>${val}</span>
  </div>`
}
...
function iniciarEnvio() {
  if (verificarCohecho(_tareas)) {
    mostrarModalAlerta(
      `Este servicio registra un procedimiento por <strong>cohecho</strong>.
       El sistema registrará una alerta crítica y notificará la cadena de mando.`,
...
async function enviarServicio() {
  ...
  // Resolver cuartel: asignado al usuario o seleccionado manualmente
  const cuartelId  = getCuartelId()
  const cuartelObj = getCuartelObj()

  if (!cuartelId) {
    toast('No hay cuartel seleccionado', 'err')
  ...
  try {
    if (!navigator.onLine) {
      ...
      mostrarExito(true); return
    }

    const { data: svc, error: svcErr } = await APP.sb
      .from('servicios').insert(svcPayload).select().single()
    ...
    for (const tarea of _tareas) {
      const { data: tar, error: tarErr } = await APP.sb.from('tareas').insert({
        ...
        hora_termino:          tarea.hora_termino          || null,
        observaciones:         tarea.observaciones         || null,
        tiene_hallazgo:        tarea.tiene_hallazgo,
        tiene_resultado:       tarea.tiene_resultado,
      ...
      if (tarea.tiene_resultado && tarea.resultado.tipo_resultado) {
        ...
        for (const e of tarea.resultado.especies) {
          await APP.sb.from('especies').insert({
            ...
            valor_uf:       e.valor_uf       || 0,
            tipo_droga:     e.tipo_droga     || null,
            cantidad_droga: parseFloat(e.cantidad_droga) || null,
            unidad_droga:   e.unidad_droga   || null,
            tipo_arma:      e.tipo_arma      || null,
            cantidad_armas: parseInt(e.cantidad_armas) || null,
            numero_serie:   e.numero_serie   || null,
            tipo_vehiculo:  e.tipo_vehiculo  || null,
            marca:          e.marca          || null,
...
function mostrarExito(offline) {
  const wrap = APP.sinCuartel
    ? el('pantalla-registro')
    : el('pantalla-registro')
  wrap.innerHTML = `
    <div class="container-sm">
      <div class="exito-wrap">
        ...
        <div class="exito-titulo">
          ${offline ? 'GUARDADO OFFLINE' : 'SERVICIO REGISTRADO'}
        </div>
        <div class="exito-sub">
          ${offline
            ? 'El servicio se sincronizará automáticamente al recuperar la conexión.'
            : 'El servicio ha sido guardado y registrado correctamente.'}
        </div>
        <button class="btn btn-primario" onclick="renderRegistro()">
