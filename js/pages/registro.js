// ============================================================
// SISCOF — Registro de Servicio (Wizard 4 pasos)
// ============================================================
...
async function renderRegistro() {
  ...
  const [{ data: p }, { data: v }, { data: pt }] = await Promise.all([
    APP.sb.from('personal').select('*').eq('cuartel_id', APP.cuartel.id).eq('activo', true).order('nombre_completo'),
    APP.sb.from('vehiculos').select('*').eq('cuartel_id', APP.cuartel.id).eq('activo', true),
    APP.sb.from('puntos_territoriales').select('*').eq('cuartel_id', APP.cuartel.id).eq('activo', true).order('tipo').order('nombre'),
  ])
  _personal  = p || []
  _vehiculos = v || []
  _puntos    = pt || []
  _paso = 1
  _servicio  = { personal_id:'', nombre_jefe:'', fecha: hoy(), turno:'', hora_inicio:'', vehiculo_id:'', patente_texto:'', km_inicio:'', hora_termino:'', km_termino:'', obs_generales:'' }
  _tareas    = []
  renderPaso()
}
...
// ── PASO 1 ────────────────────────────────────────────────────
function paso1Html() {
  const opsPersonal = _personal.map(p => `<option value="${p.id}" data-nombre="${p.grado} ${p.nombre_completo}">${p.grado} ${p.nombre_completo}</option>`).join('')
  const opsVehiculo = _vehiculos.map(v => `<option value="${v.id}" data-patente="${v.patente}">${v.patente} — ${v.tipo || ''}</option>`).join('')
  ...
  return `
    ...
    <p style="font-family:var(--mono);font-size:.68rem;color:var(--muted);margin-bottom:1.5rem">
      Cuartel: <strong style="color:var(--green)">${APP.cuartel?.nombre}</strong>
    </p>
...
function bindTareas() {
  // Los event listeners de las tareas se manejan mediante atributos onclick inline
}
...
function agregarTarea(tipo, categoria) {
  ...
  // Re-renderizar solo el timeline para no perder inputs
  const wrap = el('timeline-wrap')
  if (wrap) {
    wrap.innerHTML = `<div class="timeline" id="timeline">${_tareas.map((t, i) => tareaHtml(t, i)).join('')}</div>`
  }
}
...
function tareaHtml(t, index) {
  ...
  const necesitaResultado = ['control_vehicular_con','detencion','incautacion','hallazgo_encargo'].includes(t.tipo)
  ...
  return `
    <div class="tarea-card cat-${t.categoria}" id="tarea-${t.id}">
      ...
      <div class="g3 gap">
        <div class="campo sm">
          <label>Hora Inicio</label>
          <input type="time" value="${t.hora_inicio}" onchange="syncTarea('${t.id}','hora_inicio',this.value)" />
        </div>
        <div class="campo sm">
          <label>Hora Término</label>
          <input type="time" value="${t.hora_termino}" onchange="syncTarea('${t.id}','hora_termino',this.value)" />
        </div>
      </div>
      ...
      <div class="g2 gap">
        <div class="campo sm">
          ...
          <select onchange="syncTarea('${t.id}','institucion',this.value)">
            <option value="">Seleccionar...</option>
            ${['PNP Tacna','PNP Arica','PDI','Aduana','SML','Ejército','Otra'].map(i => `<option>${i}</option>`).join('')}
          </select>
        </div>
        <div class="campo sm">
          <label>Nivel</label>
      ...
      ${esControl ? `
      <div class="g2 gap">
        <div class="campo sm"><label>N° Vehículos</label><input type="number" min="0" value="${t.cantidad_vehiculos}" onchange="syncTarea('${t.id}','cantidad_vehiculos',+this.value)" /></div>
        <div class="campo sm"><label>N° Personas</label><input type="number" min="0" value="${t.cantidad_personas}" onchange="syncTarea('${t.id}','cantidad_personas',+this.value)" /></div>
      </div>` : ''}

      ${necesitaResultado ? resultadoHtml(t) : ''}

      <!-- Hallazgo de inteligencia -->
      <label class="check-label" style="margin-top:.75rem">
        ...
        <span style="color:var(--yellow)">¿Hay hallazgo relevante para inteligencia / CSF?</span>
      </label>
...
function toggleHallazgo(id, show) {
  const bloque = el(`hallazgo-bloque-${id}`)
  if (bloque) bloque.style.display = show ? '' : 'none'
}
...
function agregarDetenido(tareaId) {
  const t = _tareas.find(t => t.id === tareaId)
  if (!t) return
  const did = `d${Date.now()}`
  t.resultado.detenidos.push({ id: did, nacionalidad:'', edad:'', sexo:'', situacion_migratoria:'irregular' })
...
function agregarEspecie(tareaId) {
  ...
  t.resultado.especies.push({ id: eid, tipo:'droga', descripcion:'', valor_uf:0, tipo_droga:'', cantidad_droga:'', unidad_droga:'kg', tipo_arma:'', cantidad_armas:1, tiene_municion:false, numero_serie:'', monto_dinero:'', moneda:'CLP', tipo_vehiculo:'', marca:'', modelo:'', patente:'', tenia_encargo:false })
  renderDetenidosEspecies(tareaId)
}
  ...
  if (dWrap) dWrap.innerHTML = t.resultado.detenidos.map(d => `
    <div style="background:var(--bg2);padding:.6rem;border-radius:var(--radius);margin-bottom:.4rem">
      <div class="g3">
        ...
        <div class="campo sm">
          <label>Edad</label>
          <input type="number" min="0" max="99" value="${d.edad}" onchange="syncDetenido('${tareaId}','${d.id}','edad',this.value)" />
        </div>
        <div class="campo sm">
          <label>Sexo</label>
        ...
        <div class="campo sm" style="grid-column:1/3">
          <label>Situación Migratoria</label>
    ...
    <div style="background:var(--bg2);padding:.75rem;border-radius:var(--radius);margin-bottom:.4rem">
      <div class="g3 gap">
        ...
        <div class="campo sm">
          ...
          <input type="number" step="0.01" value="${e.valor_uf}" onchange="syncEspecie('${tareaId}','${e.id}','valor_uf',+this.value)" />
        </div>
...
function paso3Html() {
  return `
    ...
    <div class="g2 gap">
      <div class="campo"><label>Hora Término</label><input id="f-hora-termino" type="time" value="${_servicio.hora_termino}" /></div>
      <div class="campo"><label>Km Término</label><input id="f-km-termino" type="number" value="${_servicio.km_termino}" placeholder="0" /></div>
    </div>
...
function paso4Html() {
  ...
  return `
    ...
    <div class="resumen-lista gap2">
      ...
      ${filaResumen('Tareas',     `<span style="color:var(--green)">${_tareas.length} registradas</span>`)}
      ${conResult   ? filaResumen('Con resultado', `<span style="color:var(--red)">${conResult}</span>`) : ''}
      ${conHallazgo ? filaResumen('Hallazgos intel.', `<span style="color:var(--yellow)">${conHallazgo}</span>`) : ''}
    </div>
    <div class="aviso">⚠ Una vez enviado no puede modificarse sin autorización del Administrador.</div>
    <div class="flex-gap" style="margin-top:1.5rem">
      <button class="btn btn-secundario" onclick="_paso=3;renderPaso()">← VOLVER</button>
      <button class="btn btn-primario" id="btn-enviar" onclick="enviarServicio()">▶ ENVIAR SERVICIO</button>
    </div>
  `
}
...
async function enviarServicio() {
  ...
  try {
    // 1. Servicio
    const { data: svc, error: svcErr } = await APP.sb.from('servicios').insert({
    ...
    el('pantalla-registro').innerHTML = `
      <div class="container-sm">
        <div class="exito-wrap">
          ...
          <div class="exito-titulo">SERVICIO REGISTRADO</div>
          <div class="exito-sub">El servicio ha sido guardado correctamente.</div>
