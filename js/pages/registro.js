...
// GEO: Radio de influencia + GPS + Evaluación criticidad terreno
// ============================================================
...
const NACIONALIDADES = [
  'BOLIVIANO','PERUANO','CHILENO','VENEZOLANO',
  'COLOMBIANO','ECUATORIANO','ARGENTINO','OTRO',
]
const ESTADO_PUNTO_LABELS = {
...
function puntosMasCercanos(lat, lon, puntos, limite = 3) {
  return puntos
    ...
    .map(p => ({
      ...
      dentro_radio:  distanciaKm(lat, lon, p.latitud, p.longitud)
                     <= (p.radio_influencia_km || 10),
    }))
      ...
      <div class="container-sm">
        <div class="card gap3">
          ...
          <p style="font-size:.8rem;color:var(--muted)">
            Su usuario no tiene cuartel asignado. Seleccione el cuartel del servicio a registrar.
          </p>
          <div class="campo">
            <label>Cuartel</label>
...
async function seleccionarCuartelManual(cuartelId) {
  if (!cuartelId) return
  const todos   = await obtenerCuarteles()
  const cuartel = todos.find(c => c.id === cuartelId)
  if (!cuartel) return
  _servicio._cuartel_id_manual = cuartelId
  _servicio._cuartel_manual    = cuartel
...
function renderPaso() {
  const wizardWrap = el('wizard-wrap')
  if (wizardWrap && APP.sinCuartel) {
    wizardWrap.innerHTML = `
      ${progreso()}
      <div class="card" id="paso-contenido" style="margin-top:1rem">
...
function progreso() {
  ...
  return `<div class="progreso gap3">${labels.map((l,i) => {
...
// ── PASO 1 ────────────────────────────────────────────────────
function paso1Html() {
  const cuartelNombre = getCuartelObj()?.nombre || '—'
  return `
    <h2 style="font-family:var(--display);font-size:1.5rem;letter-spacing:3px;
      color:var(--text);margin-bottom:.25rem">PASO 1 — DATOS DEL SERVICIO</h2>
    <p style="font-family:var(--mono);font-size:.68rem;color:var(--muted);margin-bottom:1.5rem">
      Cuartel: <strong style="color:var(--verde)">${cuartelNombre}</strong>
    ...
    <div class="m5-toggle-wrap ${_m5Activo?'m5-active':''}" id="m5-wrap">
      ...
      <label class="check-label" style="margin:0">
        ...
        <span style="font-weight:700;font-size:.78rem;
          color:${_m5Activo?'#0055d4':'var(--muted)'}">
    ...
    <div class="g2 gap">
      ...
      <div class="campo">
        ...
        <input id="f-vehiculo-texto" type="text" placeholder="Ej: BPZK-45 — Camioneta"
          value="${_servicio.patente_texto}"
          onchange="_servicio.patente_texto=this.value.trim()" />
      </div>
      <div class="campo">
        <label>Km Inicio</label>
        <input id="f-km-inicio" type="number"
          value="${_servicio.km_inicio}" placeholder="0" />
      </div>
    </div>
...
function toggleM5(activo) {
  ...
  const wrap=el('m5-wrap'); const det=el('m5-detalle')
...
function paso2Html() {
  ...
  return `
    <h2 style="font-family:var(--display);font-size:1.5rem;letter-spacing:3px;
      color:var(--text);margin-bottom:.25rem">PASO 2 — TAREAS DEL SERVICIO</h2>
    <p style="font-family:var(--mono);font-size:.68rem;color:var(--muted);margin-bottom:1.25rem">
      Agregue las tareas en el orden que ocurrieron</p>
...
async function usarGPSTarea(tareaId) {
  ...
  try {
    ...
    if(el(`lat-${tareaId}`)) el(`lat-${tareaId}`).value=pos.lat
    if(el(`lon-${tareaId}`)) el(`lon-${tareaId}`).value=pos.lon
    buscarPuntosCercanos(tareaId, pos.lat, pos.lon)
    if(btn){ btn.textContent='✓ GPS capturado'; btn.style.background='var(--verde-claro)'; btn.style.color='var(--verde)' }
  } catch(e) {
...
function buscarPuntosCercanos(tareaId, lat, lon) {
  ...
  const wrap = el(`puntos-cercanos-${tareaId}`)
  if(!wrap) return
  if(!cercanos.length) {
    wrap.innerHTML=`<div style="font-size:.72rem;color:var(--muted);padding:.35rem 0">Sin puntos con coordenadas en radio de ${t.radio_influencia_km||10} km</div>`
    return
  }
  wrap.innerHTML = cercanos.map(p => {
    ...
    const seleccionado = t.punto_id === p.id
    return `
      <div onclick="seleccionarPuntoCercano('${tareaId}','${p.id}')"
        style="padding:.5rem .75rem;border-radius:7px;margin-bottom:.35rem;cursor:pointer;
          background:${seleccionado?'var(--verde-claro)':'var(--bg3)'};
          border:1.5px solid ${seleccionado?'var(--verde)':'var(--border)'};
        ...
        <div>
          ...
          <div style="font-size:.65rem;color:var(--muted)">
            <span style="background:${bgTipo};color:${colorTipo};padding:1px 5px;
              border-radius:3px;font-weight:700;margin-right:.3rem">${p.tipo.toUpperCase()}</span>
            ${p.valor_estrategico?.toUpperCase()||''}
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:.8rem;font-weight:700;
            color:${p.dentro_radio?'var(--verde)':'var(--muted2)'}">
            ${p.distancia_km} km
          </div>
          <div style="font-size:.6rem;color:${p.dentro_radio?'var(--verde)':'var(--muted2)'}">
            ${p.dentro_radio?'✓ dentro del radio':'fuera del radio'}
          </div>
        </div>
      </div>`
  }).join('')
...
function seleccionarPuntoCercano(tareaId, puntoId) {
  const t = _tareas.find(t=>t.id===tareaId); if(!t) return
  t.punto_id = puntoId
  const lat=el(`lat-${tareaId}`)?.value
  const lon=el(`lon-${tareaId}`)?.value
  if(lat&&lon) buscarPuntosCercanos(tareaId, parseFloat(lat), parseFloat(lon))
  const p = _puntos.find(p=>p.id===puntoId)
  toast(`Punto asociado: ${p?.nombre||puntoId}`, 'ok')
}
...
function tareaHtml(t, index) {
  ...
  const esTerrritorial=t.categoria==='territorial'
  const estadoInfo=ESTADO_PUNTO_LABELS[t.estado_punto||'sin_novedad']
  ...
  return `
    <div class="tarea-card cat-${t.categoria}" id="tarea-${t.id}">
      ...
      <div class="g3 gap">
        <div class="campo sm"><label>Hora Inicio</label>
          <input type="time" value="${t.hora_inicio}"
            onchange="syncTarea('${t.id}','hora_inicio',this.value)" /></div>
        <div class="campo sm"><label>Hora Término</label>
          <input type="time" value="${t.hora_termino}"
            onchange="syncTarea('${t.id}','hora_termino',this.value)" /></div>
      </div>

      <!-- ── BLOQUE GEOESPACIAL ── -->
      <div style="background:var(--azul-claro);border:1px solid #bbd3ff;border-radius:var(--radius-sm);
        padding:.75rem;margin:.5rem 0">
        <div style="font-family:var(--mono);font-size:.58rem;font-weight:700;
          letter-spacing:1.5px;color:var(--azul);margin-bottom:.5rem">
          📍 UBICACIÓN DEL EVENTO
        </div>
        <div class="g3 gap" style="margin-bottom:.5rem">
          <div class="campo sm"><label>Latitud</label>
            <input id="lat-${t.id}" type="number" step="0.0000001"
              placeholder="-18.3523" value="${t.latitud_evento||''}"
              onchange="syncTarea('${t.id}','latitud_evento',parseFloat(this.value)||null);
                buscarPuntosCercanos('${t.id}',parseFloat(this.value),parseFloat(el('lon-${t.id}')?.value)||null)" />
          </div>
          <div class="campo sm"><label>Longitud</label>
            <input id="lon-${t.id}" type="number" step="0.0000001"
              placeholder="-70.3312" value="${t.longitud_evento||''}"
              onchange="syncTarea('${t.id}','longitud_evento',parseFloat(this.value)||null);
                buscarPuntosCercanos('${t.id}',parseFloat(el('lat-${t.id}')?.value)||null,parseFloat(this.value))" />
          </div>
          <div style="display:flex;flex-direction:column;justify-content:flex-end">
            <button id="gps-btn-${t.id}" class="btn btn-secundario"
              style="font-size:.7rem;padding:.4rem .65rem;width:100%"
              onclick="usarGPSTarea('${t.id}')">
              📍 Usar GPS
            </button>
          </div>
        </div>
        <div style="font-size:.68rem;color:var(--azul);margin-bottom:.35rem;font-weight:600">
          Puntos territoriales dentro del radio:
        </div>
        <div id="puntos-cercanos-${t.id}">
          ${t.latitud_evento
            ? '<div style="font-size:.7rem;color:var(--muted)">Calculando...</div>'
            : '<div style="font-size:.7rem;color:var(--muted)">Ingrese coordenadas o use GPS para ver puntos cercanos</div>'}
        </div>
      </div>

      ${necesitaPunto ? `
      <div class="campo sm gap">
        <label>O seleccione punto directamente (si conoce el punto exacto)</label>
      ...
      ${esTerrritorial ? `
      <!-- Evaluación de criticidad en terreno -->
      <div style="background:var(--verde-claro);border:1px solid #b8e0c0;
        border-radius:var(--radius-sm);padding:.75rem;margin:.4rem 0">
        <div style="font-family:var(--mono);font-size:.58rem;font-weight:700;
          letter-spacing:1.5px;color:var(--verde);margin-bottom:.5rem">
          📊 EVALUACIÓN EN TERRENO (para CSF)
        </div>
        <div style="font-size:.72rem;font-weight:600;color:var(--text);margin-bottom:.35rem">
          Estado del punto al momento de la visita:
        </div>
        <div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-bottom:.6rem">
          ${Object.entries(ESTADO_PUNTO_LABELS).map(([val,info])=>`
            <button onclick="syncTarea('${t.id}','estado_punto','${val}');renderEstadoBtn('${t.id}')"
              id="estado-btn-${t.id}-${val}"
              style="padding:.35rem .7rem;border-radius:6px;border:1.5px solid;
                font-size:.7rem;font-weight:700;cursor:pointer;transition:all .15s;
                border-color:${info.color};
                background:${t.estado_punto===val?info.bg:'var(--bg2)'};
                color:${info.color};
                opacity:${t.estado_punto===val?1:.6}">
              ${info.label}
            </button>`).join('')}
        </div>
        <div class="g2 gap" style="margin-bottom:.4rem">
          <div class="campo sm">
            <label>Probabilidad (1–5)</label>
            <select onchange="syncTarea('${t.id}','prob_terreno',parseInt(this.value)||null);calcCriticidad('${t.id}')">
              <option value="">Seleccionar...</option>
              <option value="1" ${t.prob_terreno===1?'selected':''}>1 — Muy baja</option>
              <option value="2" ${t.prob_terreno===2?'selected':''}>2 — Baja</option>
              <option value="3" ${t.prob_terreno===3?'selected':''}>3 — Media</option>
              <option value="4" ${t.prob_terreno===4?'selected':''}>4 — Alta</option>
              <option value="5" ${t.prob_terreno===5?'selected':''}>5 — Muy alta</option>
            </select>
          </div>
          <div class="campo sm">
            <label>Consecuencia (1–5)</label>
            <select onchange="syncTarea('${t.id}','consec_terreno',parseInt(this.value)||null);calcCriticidad('${t.id}')">
              <option value="">Seleccionar...</option>
              <option value="1" ${t.consec_terreno===1?'selected':''}>1 — Menor</option>
              <option value="2" ${t.consec_terreno===2?'selected':''}>2 — Moderado</option>
              <option value="3" ${t.consec_terreno===3?'selected':''}>3 — Significativo</option>
              <option value="4" ${t.consec_terreno===4?'selected':''}>4 — Grave</option>
              <option value="5" ${t.consec_terreno===5?'selected':''}>5 — Crítico</option>
            </select>
          </div>
        </div>
        <div id="criticidad-result-${t.id}" style="font-size:.75rem;font-weight:600;
          padding:.3rem .6rem;border-radius:5px;background:var(--bg3);color:var(--muted)">
          ${t.prob_terreno&&t.consec_terreno
            ? criticidadLabel(t.prob_terreno*t.consec_terreno)
            : 'Seleccione probabilidad y consecuencia para calcular criticidad'}
        </div>
      </div>` : ''}
      ...
      <div class="g2 gap">
        <div class="campo sm"><label>Institución</label>
          <select onchange="syncTarea('${t.id}','institucion',this.value)">
            <option value="">Seleccionar...</option>
            ${['PNP Tacna','PNP Arica','PDI','Aduana','SML','Ejército','Armada','FACh','Otra']
              .map(i=>`<option>${i}</option>`).join('')}
          </select></div>
        <div class="campo sm"><label>Nivel</label>
        ...
        <div class="campo sm gc12"><label>Detalle</label>
          <input type="text" placeholder="Descripción breve"
            onchange="syncTarea('${t.id}','detalle_coordinacion',this.value)" /></div>
      </div>` : ''}
      ...
      <div class="campo sm gap"><label>Motivo</label>
        <select onchange="syncTarea('${t.id}','motivo_administrativo',this.value)">
          ...
          ${['Combustible','Traslado a ciudad','Taller / mecánico',
             'Apoyo a otra unidad','Trámite administrativo','Otro']
      ...
      <div class="g2 gap">
        <div class="campo sm"><label>N° Vehículos</label>
          <input type="number" min="0" value="${t.cantidad_vehiculos}"
            onchange="syncTarea('${t.id}','cantidad_vehiculos',+this.value)" /></div>
        <div class="campo sm"><label>N° Personas</label>
          <input type="number" min="0" value="${t.cantidad_personas}"
            onchange="syncTarea('${t.id}','cantidad_personas',+this.value)" /></div>
      </div>` : ''}
      ...
      <div id="hallazgo-bloque-${t.id}" class="hallazgo-bloque"
        style="${t.tiene_hallazgo?'':'display:none'}">
        <div class="g2 gap">
          <div class="campo sm"><label>Tipo de Evidencia</label>
            <select onchange="syncTarea('${t.id}','hal.tipo_evidencia',this.value)">
              ...
              ${['Huellas vehiculares','Huellas peatonales','Residuos recientes',
                 'Campamento','Vehículo abandonado','Señalización ilícita','Otro']
          ...
          <div class="campo sm gc12"><label>Descripción detallada</label>
            <textarea placeholder="Hora exacta, ubicación, características..."
              onchange="syncTarea('${t.id}','hal.descripcion',this.value)"
            >${t.hallazgo.descripcion}</textarea></div>
        </div>
      </div>
      ...
      <div class="campo sm" style="margin-top:.75rem">
        ...
        <input type="text" placeholder="Novedades, condiciones del terreno..."
          ...
          onchange="syncTarea('${t.id}','observaciones',this.value)" />
      </div>
    </div>`
}

function renderEstadoBtn(tareaId) {
  const t = _tareas.find(t=>t.id===tareaId); if(!t) return
  Object.keys(ESTADO_PUNTO_LABELS).forEach(val=>{
    const btn=el(`estado-btn-${tareaId}-${val}`)
    if(!btn) return
    const info=ESTADO_PUNTO_LABELS[val]
    btn.style.background  = t.estado_punto===val?info.bg:'var(--bg2)'
    btn.style.opacity     = t.estado_punto===val?'1':'.6'
  })
}

function calcCriticidad(tareaId) {
  const t = _tareas.find(t=>t.id===tareaId); if(!t) return
  const res=el(`criticidad-result-${tareaId}`)
  if(!res) return
  if(!t.prob_terreno||!t.consec_terreno) return
  const val=t.prob_terreno*t.consec_terreno
  const info=criticidadInfo(val)
  res.style.background=info.bg
  res.style.color=info.color
  res.textContent=criticidadLabel(val)
}
    ...
    <div class="resultado-bloque" id="res-bloque-${t.id}">
      ...
      <div class="g2 gap">
        ...
        <div class="campo sm"><label>N° Parte</label>
          <input type="text" placeholder="Ej: 1234/2026"
      ...
      <div class="campo sm gap"><label>¿Cómo se detectó?</label>
        <select onchange="syncTarea('${t.id}','res.como_se_detecto',this.value)">
          <option value="">Seleccionar...</option>
          ${['Patrullaje rutinario','Punto de control','Inteligencia previa',
             'Coordinación externa','Denuncia ciudadana']
...
function agregarDetenido(tareaId) {
  const t=_tareas.find(t=>t.id===tareaId); if(!t) return
  t.resultado.detenidos.push({
    id:`d${Date.now()}`,nacionalidad:'',edad:'',sexo:'',
    situacion_migratoria:'irregular',tiene_alerta_internacional:false,
  })
  renderDetenidosEspecies(tareaId)
}
function agregarEspecie(tareaId) {
  const t=_tareas.find(t=>t.id===tareaId); if(!t) return
  t.resultado.especies.push({
    id:`e${Date.now()}`,tipo:'droga',descripcion:'',valor_uf:0,
    tipo_droga:'',cantidad_droga:'',unidad_droga:'kg',
    tipo_arma:'',cantidad_armas:1,numero_serie:'',
    monto_dinero:'',moneda:'CLP',
    tipo_vehiculo:'',marca:'',modelo:'',patente:'',tenia_encargo:false,
  })
  renderDetenidosEspecies(tareaId)
}
  ...
  if(eWrap) eWrap.innerHTML=t.resultado.especies.map(e=>`
    <div style="background:var(--bg2);padding:.75rem;border-radius:var(--radius);margin-bottom:.4rem">
      <div class="g3 gap">
        <div class="campo sm"><label>Tipo</label>
          <select onchange="syncEspecie('${tareaId}','${e.id}','tipo',this.value)">
            ${['droga','dinero','vehiculo','arma','mercaderia','otro']
              .map(o=>`<option value="${o}" ${e.tipo===o?'selected':''}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join('')}
          </select></div>
      ...
      ${e.tipo==='vehiculo'?`<div class="g3">
        ...
        <div class="campo sm"><label>Marca</label><input type="text" value="${e.marca}" onchange="syncEspecie('${tareaId}','${e.id}','marca',this.value)"/></div>
...
function syncDetenido(tId,dId,campo,val){const t=_tareas.find(t=>t.id===tId);const d=t?.resultado.detenidos.find(d=>d.id===dId);if(d)d[campo]=val}
...
function syncEspecie(tId,eId,campo,val){const t=_tareas.find(t=>t.id===tId);const e=t?.resultado.especies.find(e=>e.id===eId);if(e)e[campo]=val}
...
function paso3Html() {
  return `
    <h2 style="font-family:var(--display);font-size:1.5rem;letter-spacing:3px;
      color:var(--text);margin-bottom:1.5rem">PASO 3 — CIERRE DEL SERVICIO</h2>
    <div class="g2 gap">
      ...
      <div class="campo"><label>Km Término</label>
        <input id="f-km-termino" type="number"
          value="${_servicio.km_termino}" placeholder="0" /></div>
    </div>
    <div id="aviso-nocturno" class="aviso-nocturno"></div>
    <div class="campo gap2">
      <label>Observaciones Generales</label>
      <textarea id="f-obs"
        placeholder="Condiciones climáticas, novedades menores..."
      >${_servicio.obs_generales}</textarea>
    </div>
    <div class="flex-gap" style="margin-top:1.5rem">
...
function detectarTurnoNocturno() {
  const hI=_servicio.hora_inicio||''; const hT=el('f-hora-termino')?.value||''
  const aviso=el('aviso-nocturno')
  if(!aviso||!hI||!hT) return
  if(hT<hI){
    const[hi,mi]=hI.split(':').map(Number); const[ht,mt]=hT.split(':').map(Number)
...
function paso4Html() {
  ...
  return `
    <h2 style="font-family:var(--display);font-size:1.5rem;letter-spacing:3px;
      color:var(--text);margin-bottom:1.5rem">PASO 4 — CONFIRMAR Y ENVIAR</h2>
    ${tieneCohecho?`
    <div class="alerta-cohecho" style="margin-bottom:1rem">
    ...
    <div class="resumen-lista gap2">
      ...
      ${_servicio.tiene_mision_ffaa
        ?filaResumen('M5 FF.AA.',`<span style="color:var(--azul);font-weight:600">ACTIVO — ${_servicio.unidad_ffaa||'Decreto N°78'}</span>`):''}
      ...
      ${filaResumen('Con coordenadas GPS',`<span style="color:${conGeo?'var(--verde)':'var(--muted)'}">${conGeo} de ${_tareas.length} tareas</span>`)}
      ${filaResumen('Con eval. criticidad',`<span style="color:${conCrit?'var(--verde)':'var(--muted)'}">${conCrit} evaluaciones en terreno</span>`)}
...
function filaResumen(key,val){return`<div class="resumen-fila"><span class="resumen-key">${key}</span><span>${val}</span></div>`}

function iniciarEnvio(){
  if(verificarCohecho(_tareas)){
    mostrarModalAlerta(
      `Este servicio registra un procedimiento por <strong>cohecho</strong>. Se registrará una alerta crítica.`,
      (confirmado)=>{ if(confirmado) enviarServicio() }
    )
  } else { enviarServicio() }
}

// ── Envío completo con datos geoespaciales ───────────────────
async function enviarServicio() {
  ...
  const svcPayload={
    cuartel_id:cuartelId,nombre_jefe:_servicio.nombre_jefe,
    fecha:_servicio.fecha,turno:_servicio.turno,
    hora_inicio:_servicio.hora_inicio,hora_termino:_servicio.hora_termino||null,
    ...
    duracion_minutos:durNocturna||null,
    estado:'enviado',
  }
  try {
    ...
    for(const tarea of _tareas){
      const{data:tar,error:tarErr}=await APP.sb.from('tareas').insert({
        servicio_id:svc.id,cuartel_id:cuartelId,
        ...
        // GEO
        latitud_evento:tarea.latitud_evento||null,
        longitud_evento:tarea.longitud_evento||null,
        prob_terreno:tarea.prob_terreno||null,
        consec_terreno:tarea.consec_terreno||null,
      ...
      if(tarea.tiene_resultado&&tarea.resultado.tipo_resultado){
        ...
        if(tarea.resultado.tipo_delito==='cohecho'){
          await registrarAlertaCritica('cohecho',
            `Servicio ${svc.id} — Parte N°${tarea.resultado.numero_parte||'S/N'}`,cuartelId)
        }
