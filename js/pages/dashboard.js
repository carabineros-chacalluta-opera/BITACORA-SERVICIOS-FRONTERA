// ============================================================
// SISCOF — Dashboard con IDFI, DFP y DFO
...
function idfiConsolidadoHtml(datos) {
  const anioAnt  = new Date().getFullYear() - 1
  // Usar primer cuartel si es vista individual, o promedio si es global
  ...
  // Comparativa IDFI
  const idfi_hist  = r.idfi_hist
  const dfp_hist   = r.dfp_hist
  const dfo_hist   = r.dfo_hist
  const sin_hist   = r.sin_idfi_hist

  const varIdfi = sin_hist || idfi_hist === null ? null : idfi - idfi_hist
  const varDfp  = sin_hist || dfp_hist  === null ? null : dfp  - dfp_hist
  const varDfo  = sin_hist || dfo_hist  === null ? null : dfo  - dfo_hist

  // Diagnóstico 2×2 del Informe
  const dfpAlta = dfp >= 70
  const dfoAlta = dfo >= 10  // DFO mide tasas, umbrales naturalmente bajos
  ...
  return `
    <div class="card gap3" style="border-left:4px solid ${color};background:${bgCol}">
      <div class="flex-sb flex-wrap" style="gap:1rem;align-items:flex-start">

        <!-- IDFI principal -->
        <div>
          <div class="sec-titulo" style="margin-bottom:.3rem">
            ÍNDICE DE DESEMPEÑO FRONTERIZO INTEGRAL — Informe Técnico Nro. 01/2026
          </div>
          <div style="display:flex;align-items:baseline;gap:.75rem;flex-wrap:wrap">
            ...
            ${varIdfi !== null
              ? `<span style="font-size:.85rem">${varHtml(varIdfi, false, ' pts')} vs ${anioAnt} ${pctHtml(idfi, idfi_hist)}</span>`
              : `<span class="sin-dato">Sin dato ${anioAnt}</span>`}
          </div>
          <!-- Barra bicolor DFP/DFO -->
        ...
        <div>
          ...
          <div style="background:${diagBg};border:1.5px solid ${diagColor}33;border-radius:var(--radius-sm);
            padding:.65rem 1rem;font-size:.8rem;font-weight:600;color:${diagColor};max-width:280px">
            ${diagnostico}
          </div>
...
function matrizCelda(label, estado, activo, color) {
  return `
    <div style="padding:5px 8px;border-radius:4px;font-size:.6rem;font-weight:${activo?700:400};
    ...
    <div class="g2 gap3" style="align-items:start">
      ...
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem">
          ...
          <div style="text-align:right">
            <div style="font-size:1.6rem;font-weight:700;color:var(--verde);letter-spacing:-.5px">
        ...
        ${indicadorFila('DFP-01','Hitos Fronterizos','Peso 25%',  r.dfp.dfp01, r.dfp01_hist, sin, '#1a6b2a', '0.25')}
        ${indicadorFila('DFP-02','PNH Fiscalizados', 'Peso 30%',  r.dfp.dfp02, r.dfp02_hist, sin, '#1a6b2a', '0.30')}
        ${indicadorFila('DFP-03','SIE Visitados',    'Peso 15%',  r.dfp.dfp03, r.dfp03_hist, sin, '#2a8a3e', '0.15')}
        ${indicadorFila('DFP-04','Coordinación Int.','Peso 15%',  r.dfp.dfp04, r.dfp04_hist, sin, '#2a8a3e', '0.15')}
        ${indicadorFila('DFP-05','Prod. Inteligencia','Peso 15%', r.dfp.dfp05, r.dfp05_hist, sin, '#2a8a3e', '0.15')}
      ...
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem">
          ...
          <div style="text-align:right">
            <div style="font-size:1.6rem;font-weight:700;color:var(--azul);letter-spacing:-.5px">
        ...
        ${indicadorFila('DFO-01','Eficacia Controles',  'Peso 15%', r.dfo.dfo01, r.dfo01_hist, sin, '#0055d4', '0.15')}
        ${indicadorFila('DFO-02','Docs. Falsificados',  'Peso 10%', r.dfo.dfo02, r.dfo02_hist, sin, '#0055d4', '0.10')}
        ${indicadorFila('DFO-03','Control Migratorio',  'Peso 15%', r.dfo.dfo03, r.dfo03_hist, sin, '#0055d4', '0.15')}
        ${indicadorFila('DFO-04','Interdicción CT',     'Peso 30%', r.dfo.dfo04, r.dfo04_hist, sin, '#d70015', '0.30')}
        ${indicadorFila('DFO-05','Impacto Económico UF','Peso 15%', r.dfo.dfo05, r.dfo05_hist, sin, '#9a6e00', '0.15')}
        ${indicadorFila('DFO-06','Obj. Internacionales','Peso 15%', r.dfo.dfo06, r.dfo06_hist, sin, '#c45000', '0.15')}
...
function indicadorFila(codigo, nombre, peso, valor, histVal, sinHist, color, pesoNum) {
  const pct   = (valor || 0).toFixed(1)
  const nivel = valor >= 90 ? 'ÓPTIMO' : valor >= 70 ? 'ADECUADO'
    : valor >= 50 ? 'DEFICIENTE' : 'CRÍTICO'
        ...
        <div style="flex:1">
          <div style="display:flex;align-items:center;gap:.5rem">
            <span style="font-family:var(--mono);font-size:.58rem;font-weight:700;
              color:${color};background:${color}18;padding:2px 6px;border-radius:3px">
              ${codigo}
            </span>
...
// ════════════════════════════════════════════════════════════
// MÉTRICAS DFO BRUTAS (4 KPIs superiores)
// ════════════════════════════════════════════════════════════
    ...
    lista.forEach(c => {
      ...
      rows += `
        <tr class="${alerta ? 'fila-alerta' : ''}" onclick="toggleDetalle('${c.id}')">
          ...
          <td>
            <span style="font-weight:700;color:${color}">${idfi.toFixed(0)}%</span>
            <span style="font-size:.6rem;color:${color};margin-left:3px">${label_}</span>
          </td>
