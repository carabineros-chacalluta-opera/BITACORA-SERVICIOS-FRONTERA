...
// FIX: usa getCuartelId() para soportar usuario sin cuartel
// ============================================================

function renderAdmin() {
  if (!APP.esAdmin) {
    el('pantalla-admin').innerHTML = `<div class="cargando">SIN ACCESO</div>`
    return
  }
  ...
  el('pantalla-admin').innerHTML = `
    <div class="container" style="max-width:860px">
      ...
      <div class="card gap3" style="margin-bottom:1.25rem">
        ...
        <button class="btn btn-primario" onclick="guardarDenominadores()">
      ...
      <div class="card gap3" style="margin-bottom:1.25rem">
        ...
        <p style="font-size:.82rem;color:var(--muted)">
          Guarda el IDFI calculado del mes en el historial
          para comparar con el año siguiente.
        </p>
        <div class="g2 gap2">
          <div class="campo"><label>Año</label>
            <input id="admin-anio" type="number"
              value="${ahora.getFullYear()}" min="2020" max="2099" /></div>
          <div class="campo"><label>Mes</label>
            <select id="admin-mes">
              ${['Enero','Febrero','Marzo','Abril','Mayo','Junio',
                 'Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']
                .map((m,i) =>
                  `<option value="${i+1}" ${i+1===ahora.getMonth()?'selected':''}>${m}</option>`)
                .join('')}
            </select></div>
        </div>
        <div class="campo"><label>Cuartel a cerrar</label>
          <select id="admin-cuartel-cierre">
      ...
      <div class="card gap3">
        ...
        <div class="g2 gap">
          <div class="campo"><label>Nombre de la unidad</label>
            <input id="cfg-unidad" type="text"
              value="${SISCOF_CONFIG.NOMBRE_UNIDAD}" /></div>
          <div class="campo"><label>Nombre de la institución</label>
            <input id="cfg-inst" type="text"
              value="${SISCOF_CONFIG.NOMBRE_INSTITUCION}" /></div>
        </div>
        <div style="display:flex;gap:.75rem;flex-wrap:wrap">
          <button class="btn btn-secundario" onclick="verificarConexion()">
            Verificar conexión Supabase
          </button>
          <button class="btn btn-secundario" onclick="limpiarCacheOffline()">
  ...
  cargarDenominadores()
  cargarCuartelesParaCierre()
}

async function cargarCuartelesParaCierre() {
  const cuarteles = await obtenerCuarteles()
  const sel       = el('admin-cuartel-cierre')
  if (!sel) return
  sel.innerHTML = cuarteles
...
async function cargarDenominadores() {
  ...
  if (!wrap) return
  try {
    const { data: cuarteles } = await APP.sb
      .from('cuarteles').select('*').eq('activo', true)
      .order('tipo').order('nombre')
    if (!cuarteles?.length) {
      wrap.innerHTML = '<p style="font-size:.8rem;color:var(--muted)">Sin cuarteles.</p>'
      return
    }
      ...
      <div class="denom-card" style="margin-bottom:.75rem">
        ...
        <div class="denom-grid">
          <div class="campo"><div class="denom-label">Total Hitos</div>
            <input class="denom-input" type="number"
              id="d-hitos-${c.id}" value="${c.total_hitos||0}" min="0" /></div>
          <div class="campo"><div class="denom-label">Total PNH</div>
            <input class="denom-input" type="number"
              id="d-pnh-${c.id}" value="${c.total_pnh||0}" min="0" /></div>
          <div class="campo"><div class="denom-label">Total SIE</div>
            <input class="denom-input" type="number"
              id="d-sie-${c.id}" value="${c.total_sie||0}" min="0" /></div>
          <div class="campo"><div class="denom-label">Meta UF / período</div>
            <input class="denom-input" type="number"
              id="d-uf-${c.id}" value="${c.meta_uf_periodo||500}" min="1" /></div>
          <div class="campo"><div class="denom-label">Meta coordinaciones / mes</div>
            <input class="denom-input" type="number"
              id="d-coord-${c.id}" value="${c.meta_coordinaciones_mes||4}" min="1" /></div>
          <div class="campo"><div class="denom-label">Meta hallazgos / mes</div>
            <input class="denom-input" type="number"
              id="d-hall-${c.id}" value="${c.meta_hallazgos_mes||4}" min="1" /></div>
          <div class="campo"><div class="denom-label">Meta objetivos int. / período</div>
            <input class="denom-input" type="number"
              id="d-obj-${c.id}" value="${c.meta_objetivos_int||1}" min="1" /></div>
        </div>
      </div>`).join('')
  } catch(e) {
    wrap.innerHTML = `<p style="font-size:.8rem;color:var(--rojo)">Error: ${e.message}</p>`
  }
}
...
async function guardarDenominadores() {
  ...
  if (res) res.innerHTML =
    `<span style="color:var(--verde)">✓ ${ok} cuarteles actualizados correctamente</span>`
...
async function ejecutarCierreMes() {
  ...
  const cuartelId = el('admin-cuartel-cierre')?.value
  const res       = el('admin-resultado')

  if (!cuartelId) { toast('Seleccione un cuartel', 'err'); return }
  if (res) res.textContent = 'Calculando...'

  try {
    const r = await cerrarMesIDFI(cuartelId, anio, mes)
    if (res) res.innerHTML =
...
async function verificarConexion() {
  ...
  try {
    const { error } = await APP.sb.from('cuarteles').select('count').limit(1)
    if (error) throw error
    if (res) res.innerHTML =
      `<span style="color:var(--verde)">✓ Conexión con Supabase OK</span>`
  } catch(e) {
    if (res) res.innerHTML =
      `<span style="color:var(--rojo)">✕ Error: ${e.message}</span>`
  }
}
